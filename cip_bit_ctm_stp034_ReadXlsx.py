    ...
    for line in open(arq_csv):
        ...
        if ( dtline == diajob ) and ( ws[jobcell].value in line ) and (ws[hrinicio].value in line):
            ...
            if ( mthline == mthtday ):
                ...
                monthout = datetime.datetime.today().strftime('%y%m%d')
                dest_filename1 = 'Bitacora'+monthout+'.xlsx'
                #wb.save(filename = dest_filename1)
                print ("Linha {} Arquivo {} gravado com sucesso.".format(row, dest_filename1))
                print("###################################################################")
            else:
...
################################################
    ...
    if ( row == conta and ontem == 0 and interno == 0 ):
        print ("DIAJOB ANTES {} ROW1 {} CONTA {} ONTEM {} INTERNO {}".format(diajob, row, conta, ontem, interno))
        yest=datetime.date.fromordinal(datetime.date.today().toordinal()-1)
    ...
    if ( row == conta and ontem == 0 and interno == 1 ):
        print ("DIAJOB1 ANTES {} TVT ROW1 {} CONTA {} ONTEM {} INTERNO {}".format(diajob, row, conta, ontem, interno))
        yest=datetime.date.fromordinal(datetime.date.today().toordinal()-1)
        diajob=str(yest.strftime('%Y-%m-%d'))
        row = 3
        ontem = 1
    ...
    if ( row == conta and ontem == 1 and interno == 1 ):
        print ("TVT DIAJOB ANTES {} ROW1 {} CONTA {} ONTEM {} INTERNO {}".format(diajob, row, conta, ontem, interno))
        print ("PASSO 04")
    if ( ciclico == 1 ):
